...
function mineDecolarData (DecolarDataJSON) {
  ...
  // Obtem items
  let items = [];
  for (let i = 0; i < DecolarDataJSON.result.data.items.length; i++) {
    let item = DecolarDataJSON.result.data.items[i];

    for(let j = 0; j < item.itinerariesBox.outboundRoutes.length; j++) {
      let outboundRoute = item.itinerariesBox.outboundRoutes[j];
  ...
  // Obtem menor preço
  let menorPreco = R.reduce(
    R.minBy(function (x) { return x.emissionPrice.total.fare.raw; }),
    data_scraped.Items[0], data_scraped.Items);
